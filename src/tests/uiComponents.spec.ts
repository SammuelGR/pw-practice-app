...
test.describe("Form Layouts page", () => {
  ...
  test("input fields @smoke", async ({ page }) => {
    ...
    await usingtheGridEmailInput.pressSequentially("test2@test.com", {
      delay: 500,
    }); //pressSequentially is used to write sequentially with delay we can add
  ...
  test("radio buttons", async ({ page }) => {
    const usingtheGridRadio = page.locator("nb-card", {
      hasText: "Using the Grid",
    });
    await usingtheGridRadio.getByLabel("Option 1").check({ force: true });
    //Or
    await usingtheGridRadio
      .getByRole("radio", { name: "Option 1" })
      .check({ force: true });

    const radioStatus = await usingtheGridRadio
      .getByLabel("Option 1")
      .isChecked();
